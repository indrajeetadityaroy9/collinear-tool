...
def stub_hf_and_redis(monkeypatch):
    ...
    def fake_get_page(limit, offset, search, sort_by, sort_order):
        if offset >= 10:
            ...
            emptied.update(
                {"items": [], "current_page": 0, "next_page": None, "prev_page": None}
            )
            return emptied, 200
    ...
    monkeypatch.setattr(
        "app.api.routes.datasets.get_datasets_page_from_cache", fake_get_page
    )
    monkeypatch.setattr(
        "app.api.routes.datasets.get_dataset_commits_async", fake_get_commits
    )
    monkeypatch.setattr(
        "app.api.routes.datasets.get_dataset_files_async", fake_get_files
    )
...
def test_get_file_url_valid(client):
    resp = client.get(
        "/api/datasets/openbmb/Ultra-FineWeb/file-url", params={"filename": "README.md"}
    )
    assert resp.status_code == 200
...
def test_get_file_url_invalid_file(monkeypatch, client):
    ...
    resp = client.get(
        "/api/datasets/openbmb/Ultra-FineWeb/file-url",
        params={"filename": "not_a_real_file.txt"},
    )
