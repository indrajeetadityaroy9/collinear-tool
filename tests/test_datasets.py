...
def fake_redis(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.integrations.hf_datasets.get_redis_sync", lambda: redis_instance
    )
...
def test_get_datasets_page_from_cache_filters_search(fake_redis):
    ...
    result, status = hf_datasets.get_datasets_page_from_cache(
        limit=5, offset=0, search="beta"
    )
...
def test_determine_impact_level_by_size():
    level, reason = hf_datasets.determine_impact_level_by_criteria(
        15 * 1024 * 1024 * 1024
    )
    assert level == "high"
