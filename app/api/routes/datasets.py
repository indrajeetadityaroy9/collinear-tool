...
async def list_datasets(
    ...
    search: Optional[str] = Query(
        None, description="Filter by dataset id or description"
    ),
    sort_by: Optional[str] = Query(
        None, description="Field to sort by (e.g. downloads, likes)"
    ),
    ...
    result, status_code = get_datasets_page_from_cache(
...
async def get_commits(dataset_id: str) -> List[CommitInfo]:
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Could not fetch commits: {exc}"
        ) from exc
...
async def list_files(dataset_id: str) -> List[str]:
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Could not list files: {exc}"
        ) from exc


@router.get("/{dataset_id:path}/file-url")
async def get_file_url_endpoint(
    dataset_id: str, filename: str = Query(...), revision: Optional[str] = None
) -> Dict[str, str]:
