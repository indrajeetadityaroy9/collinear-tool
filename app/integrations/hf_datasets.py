...
def _stringify_for_stream(item):
    payload = {}
...
async def _fetch_size(session, dataset_id, token=None):
...
def process_datasets_page(offset, limit):
    ...
    if stored:
        redis_client.delete(REDIS_BLOB_KEY)
...
async def get_dataset_commits_async(dataset_id, limit=20):
    ...
    for commit in commits[:limit]:
        try:
            ...
            result.append({'id': commit_id or '', 'title': title or message or '', 'message': message or title or '', 'author': {'name': author_name, 'email': ''}, 'date': date})
        except Exception as exc:
...
def get_datasets_page_from_cache(limit, offset, search=None, sort_by=None, sort_order='desc'):
    ...
    if needs_full_scan:
        ...
        if search:
            ...
            items = [item for item in items if lowered in (item.get('id') or '').lower() + ' ' + str(item.get('description') or '').lower()]
        if sort_by:
